...
if __name__ == "__main__":
    ...
    if args.accumulation == 0:
        ...
        if args.defense == 'Gauss':
            for i in range(len(input_gradient)):
                grad_tensor = input_gradient[i].cpu().numpy()
                noise = np.random.normal(0,args.noise_sigma, size=grad_tensor.shape)
                grad_tensor = grad_tensor + noise
                input_gradient[i] = torch.Tensor(grad_tensor).to(**setup)
        if args.defense == 'Lap':
            for i in range(len(input_gradient)):
                grad_tensor = input_gradient[i].cpu().numpy()
                noise = np.random.laplace(0,args.noise_sigma, size=grad_tensor.shape)
                grad_tensor = grad_tensor + noise
                input_gradient[i] = torch.Tensor(grad_tensor).to(**setup)
    ...
    if args.save_image and not args.dryrun:
        ...
        if args.defense == 'Gauss' or args.defense == 'Lap':
            rec_filename = (f'{validloader.dataset.classes[labels][0]}_{"trained" if args.trained_model else ""}'
                            f'{args.model}_{args.cost_fn}-{args.target_id}-{args.defense}-{args.noise_sigma}.png')
